...
const dynamicRoutingUtils = require("@sap/approuter/lib/utils/dynamic-routing-utils");
...
module.exports = function(req, res, next) {
  if (
    ...
    req.routerConfig.appConfig.logout.logoutEndpoint &&
    dynamicRoutingUtils
      .getCoreUrl(req)
      .split("?")
      .shift() ===
      "/custom" + req.routerConfig.appConfig.logout.logoutEndpoint
  ) {
    let app = req.app;
    ...
    let uaaRedirect = true;
    if (req.query["uaa-redirect"] === "false") {
      uaaRedirect = false;
    }

    logoutProvider.callBackendLogoutPaths(
      req,
      app.get("mainRouterConfig"),
      session
    );
    ...
    res.setHeader(
      "Set-Cookie",
      "SMADSESSION=LOGGEDOFF; path=/; HttpOnly; domain=.toyota.ca; expires=Thu, 01 Jan 1970 00:00:00 GMT"
    );
